...
异步派发元数据标签写入。`tags` 是一个 field→value 的对象；当前响应只是派发回执，最终结果通过 `metadata:writeComplete` 到达。
...
设置曲目的 0 到 5 整数评分，`0` 表示清除评分。`options.cueIndex` 可显式选择 CUE 分轨，其优先级高于路径中的 `|subsong:N` 后缀。
