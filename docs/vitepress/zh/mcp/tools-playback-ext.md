...
| 工具 | Bridge 方法 | 参数 | 说明 |
| --- | --- | --- | --- |
| `fb2k_playback_mute` | `playback.mute` | 可选 `muted: boolean`，默认为 `true` | 设置静音状态 |
| `fb2k_playback_toggle_mute` | `playback.toggleMute` | 无 | 切换静音状态 |
| `fb2k_playback_volume_up` | `playback.volumeUp` | 无 | 将音量提高一个步进 |
| `fb2k_playback_volume_down` | `playback.volumeDown` | 无 | 将音量降低一个步进 |
...
| 参数 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| includeTrackInfo | boolean | 否 | 是否包含曲目详情（默认 `false`） |
...
| 参数 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| path | string | 是 | 文件路径，支持 `path|subsong:N` 格式 |
